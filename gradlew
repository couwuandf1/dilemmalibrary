...
if $cygwin ; then
    ...
    for i do
        EQUALS=`expr $i : '.*=\(.*\)'`
        if [ $i = `expr $i : '\(.*\)='` ] && [ $EQUALS != "" ] && [ "$OURCYGPATTERN" = "" ] ; then
            i=`echo $i | sed "s|=$EQUALS|==$EQUALS|"`
        fi
        if echo $i | grep -q -E -- "$OURCYGPATTERN"; then
            i=`echo $i | sed -r 's|'"$OURCYGPATTERN"'|&|g'`
        ...
        i=`echo "$i" | sed -e 's|\\\\|/|g' -e 's|//*|/|g' -e 's|^/||'`
        i=`echo $i | sed "s|^/[a-zA-Z]:/|&|"`
        i=`echo $i | sed -e 's|^/||' -e 's|//*|/|g'`
        i="$APP_HOME/$i"
        echo "$i"
    done
}
